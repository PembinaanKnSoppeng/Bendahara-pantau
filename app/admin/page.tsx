...
import {
  ...
  Clock, Eye, Activity, RotateCcw, PenTool, Utensils, Star
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    const { data: ratingData } = await supabase.from("rating_kepuasan").select("*").eq("jenis", "Tukin").order("created_at", { ascending: false });
  ...
  const handleUpdate = async (updates: Partial<StatusTukin>) => {
    setSaving(true);
    await supabase.from("status_tukin_global").update({ ...updates, updated_at: new Date().toISOString() }).eq("id", 1);
    await fetchStatus();
    setSaving(false);
  };
  ...
  const printPDF = () => {
    ...
    const content = `
      <html>
        <head>
          <title>Laporan Kepuasan SIMantu</title>
          <style>
            @media print {
            ...
            .avg { font-size: 36pt; font-weight: 900; color: #4f46e5; margin: 0; line-height: 1;}
            table { width: 100%; border-collapse: collapse; margin-top: 20px; }
            th { background: #4f46e5; color: white; text-align: left; padding: 10px; font-size: 12px; text-transform: uppercase; }
        ...
        <body>
          ...
          <div class="stat-box">
            <div>
              <div style="font-size: 10px; font-weight: bold; color: #94a3b8; text-transform: uppercase;">Rata-rata Rating (Tukin)</div>
        ...
        <div className="p-8 flex-1 overflow-y-auto">
          ...
          <nav className="space-y-3 mb-10">
            <div className="bg-indigo-50 text-indigo-700 p-4 rounded-2xl flex items-center gap-3 border border-indigo-100 shadow-sm">
              <Activity size={18} />
              <span className="font-bold text-sm tracking-wide">Tunjangan Kinerja</span>
            </div>

            <Link href="/admin/uang-makan" className="text-slate-500 p-4 rounded-2xl flex items-center gap-3 hover:text-slate-900 hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-all font-bold text-sm group">
              <Utensils size={18} className="group-hover:text-amber-500 transition-colors"/>
              Uang Makan
            </Link>
            ...
            <div className="space-y-4">
              {STEPS.map((step, i) => (
                <div key={i} className="flex items-center gap-3.5 relative">
                  ...
                  <div className={`h-6 w-6 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 ${i + 1 < safeStep ? "bg-emerald-100 text-emerald-600" : i + 1 === safeStep ? "bg-indigo-600 text-white shadow-md shadow-indigo-200 scale-110" : "bg-white border border-slate-200 text-slate-400"}`}>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 space-y-8">
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] overflow-hidden relative animate-fade-slide-up">
            ...
            <div className="px-6 sm:px-10 pt-10 pb-6 flex flex-col md:flex-row items-start md:items-center justify-between relative z-10 gap-6">
              <div>
                ...
                <h2 className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight text-balance">
                  Tahap <span className="tabular-nums">{safeStep}</span>: <span className="text-indigo-600 block sm:inline mt-1 sm:mt-0">{STEPS[safeStep - 1]?.title}</span>
                </h2>
              </div>
            ...
            <div className="px-6 sm:px-10 mb-10 relative z-10">
              <div className="h-4 bg-slate-100 rounded-full overflow-hidden shadow-inner p-0.5">
                <div className="h-full bg-gradient-to-r from-indigo-400 to-violet-500 rounded-full transition-all duration-1000 ease-out relative" style={{ width: `${(safeStep / 8) * 100}%` }}>
            ...
            <div className="mx-4 sm:mx-10 mb-10 bg-slate-50 rounded-[2rem] p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 border border-slate-100 relative z-10">
              ...
              <button onClick={() => askConfirm({ title: "Lanjut ke Tahap Berikutnya?", desc: `Proses akan maju ke Tahap ${safeStep + 1}: ${STEPS[safeStep]?.title ?? "Selesai"}. Pastikan berkas tahap ini sudah selesai.`, confirmLabel: "Ya, Lanjut", confirmClass: "bg-indigo-600 hover:bg-indigo-700 shadow-indigo-500/30", action: async () => { await handleUpdate({ current_step: Math.min(8, safeStep + 1), is_rejected: false }); addHistory({ step: safeStep + 1, label: `Maju ke Tahap ${safeStep + 1}`, type: "advance" }); } })} disabled={safeStep === 8 || saving} className="w-full sm:w-auto px-10 py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-[0_8px_20px_-6px_rgba(79,70,229,0.5)] hover:bg-indigo-500 hover:-translate-y-1 hover:shadow-[0_12px_25px_-6px_rgba(79,70,229,0.6)] transition-all duration-300 active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div className="space-y-6 sm:space-y-8 animate-fade-slide-up" style={{ animationDelay: "200ms" }}>
              ...
              {/* KOTAK RATING & KEPUASAN (TUKIN) */}
              <div className="bg-gradient-to-br from-indigo-500 to-violet-600 rounded-[2.5rem] p-8 shadow-lg text-white mb-8 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-bl-full pointer-events-none" />
                <h3 className="font-black text-sm uppercase tracking-widest mb-6 flex items-center gap-2 relative z-10"><Star size={16} className="text-amber-300" /> Indeks Kepuasan</h3>
                <div className="flex items-end gap-4 mb-6 relative z-10">
                  ...
                  <div className="pb-2">
                    ...
                    <p className="text-xs font-medium text-indigo-100">Dari {ratings.length} responden</p>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-[2.5rem] border-2 border-rose-100 p-8 shadow-sm relative overflow-hidden group">
                ...
                <button onClick={() => askConfirm({ title: "Mulai Bulan Baru (Reset)?", desc: "Jika berada di Tahap 8, periode akan diarsipkan. Lalu progres direset ke Tahap 1.", confirmLabel: "Ya, Reset Total", confirmClass: "bg-rose-600 hover:bg-rose-700 shadow-rose-500/30", action: async () => { 
                    if (safeStep === 8) { await supabase.from("arsip_pencairan").insert([{ periode: data?.periode, jenis: "Tukin" }]); }
