...
export default function AdminUangMakanDashboard() {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    // Ambil Data Rating Khusus Uang Makan
  ...
  const downloadCSV = () => {
    const headers = ["Periode,Jenis Layanan,Bintang Diberikan,Tanggal Input\n"];
    const csvContent = "data:text/csv;charset=utf-8," + headers + ratings.map(e => `${e.periode},${e.jenis},${e.rating} Bintang,${new Date(e.created_at).toLocaleDateString("id-ID")}`).join("\n");
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `Laporan_Kepuasan_UangMakan_${new Date().getTime()}.csv`);
      ...
      <aside className={`fixed h-full top-0 left-0 z-50 w-[20rem] bg-white border-r border-slate-100 flex flex-col transition-transform duration-500 ease-out shadow-[4px_0_24px_rgba(0,0,0,0.02)] ${sidebarOpen ? "translate-x-0" : "-translate-x-full xl:translate-x-0"}`}>
        <div className="p-8 flex-1 overflow-y-auto">
          ...
          <nav className="space-y-3 mb-10">
            ...
            <a href="/uang-makan" target="_blank" rel="noreferrer" className="text-slate-500 p-4 rounded-2xl flex items-center gap-3 hover:text-slate-900 hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-all font-bold text-sm group">
              ...
              {STEPS.map((step, i) => (
                <div key={i} className="flex items-center gap-3.5 relative">
                  ...
                  <div className={`h-6 w-6 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 ${
                    ...
                    i + 1 === safeStep ? "bg-amber-500 text-white shadow-md shadow-amber-200 scale-110" :
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 space-y-8">
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] overflow-hidden relative animate-fade-slide-up">
            ...
            <div className="mx-4 sm:mx-10 mb-10 bg-slate-50 rounded-[2rem] p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 border border-slate-100 relative z-10">
              <button 
                onClick={() => askConfirm({ title: "Mundur ke Tahap Sebelumnya?", desc: `Proses akan dikembalikan ke Tahap ${safeStep - 1}: ${STEPS[safeStep - 2]?.title ?? "—"}.`, confirmLabel: "Ya, Mundur", confirmClass: "bg-orange-500 hover:bg-orange-600 shadow-orange-500/30", action: async () => { await handleUpdate({ current_step: Math.max(1, safeStep - 1), is_rejected: false }); addHistory({ step: safeStep - 1, label: `Mundur ke Tahap ${safeStep - 1}`, type: "back" }); } })} 
              ...
              <button 
                onClick={() => askConfirm({ title: "Lanjut ke Tahap Berikutnya?", desc: `Proses akan maju ke Tahap ${safeStep + 1}: ${STEPS[safeStep]?.title ?? "Selesai"}. Pastikan berkas tahap ini sudah selesai.`, confirmLabel: "Ya, Lanjut", confirmClass: "bg-amber-500 hover:bg-amber-600 shadow-amber-500/30", action: async () => { await handleUpdate({ current_step: Math.min(8, safeStep + 1), is_rejected: false }); addHistory({ step: safeStep + 1, label: `Maju ke Tahap ${safeStep + 1}`, type: "advance" }); } })} 
                disabled={safeStep === 8 || saving} 
                className="w-full sm:w-auto px-10 py-4 bg-amber-500 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-[0_8px_20px_-6px_rgba(245,158,11,0.5)] hover:bg-amber-600 hover:-translate-y-1 hover:shadow-[0_12px_25px_-6px_rgba(245,158,11,0.6)] transition-all duration-300 active:translate-y-0 disabled:opacity-50 flex items-center justify-center gap-2"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div className="space-y-6 sm:space-y-8 animate-fade-slide-up" style={{ animationDelay: "200ms" }}>
              ...
              {/* KOTAK RATING & KEPUASAN (TEMA AMBER) */}
              ...
              <div className="bg-white rounded-[2.5rem] border-2 border-rose-100 p-8 shadow-sm relative overflow-hidden group">
                ...
                <button 
                  onClick={() => askConfirm({ title: "Mulai Bulan Baru (Reset)?", desc: "Jika berada di Tahap 8, periode akan diarsipkan. Lalu progres direset ke Tahap 1.", confirmLabel: "Ya, Reset Total", confirmClass: "bg-rose-600 hover:bg-rose-700 shadow-rose-500/30", action: async () => { 
                    if (safeStep === 8) {
                      await supabase.from("arsip_pencairan").insert([{ periode: data.periode, jenis: "Uang Makan" }]);
                    }
