...
export default function PublicPage() {
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#F8FAFC] gap-6">
      <div className="relative flex items-center justify-center"><div className="absolute inset-0 animate-ping rounded-full bg-emerald-400 opacity-20 scale-[2.5]" /><div className="h-14 w-14 border-4 border-emerald-100 border-t-emerald-500 rounded-full animate-spin shadow-sm" /></div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8FAFC] font-sans selection:bg-emerald-100 selection:text-emerald-900 relative overflow-hidden">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-md bg-white rounded-[2.5rem] p-8 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-slate-100 animate-fade-slide-up">
            ...
            <div className="flex items-center gap-3 mb-6">
              <div className="bg-emerald-50 p-3 rounded-2xl text-emerald-500"><History size={24} /></div>
              <h3 className="text-xl font-black text-slate-900 tracking-tight">Riwayat Pencairan</h3>
            </div>
              ...
              {arsipData.length === 0 ? (
                ...
                arsipData.map((arsip, idx) => (
                  <div key={idx} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <div className="bg-emerald-100 text-emerald-600 p-2 rounded-full"><Check size={16} strokeWidth={3}/></div>
                  </div>
                ))
              )}
            </div>
      ...
      {showHelpModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative w-full max-w-sm bg-white rounded-[2.5rem] p-8 sm:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-slate-100 flex flex-col items-center text-center animate-fade-slide-up">
            ...
            <div className="w-20 h-20 bg-emerald-50 rounded-full flex items-center justify-center mb-6 shadow-inner border border-emerald-100/50"><HelpCircle size={36} className="text-emerald-500 animate-wiggle" /></div>
            ...
            <button onClick={() => window.open('https://wa.me/6281234567890?text=Halo%20Tim%20Keuangan,%20saya%20ingin%20bertanya%20terkait%20Tukin...', '_blank')} className="w-full py-4 bg-emerald-500 text-white rounded-2xl text-xs font-bold uppercase tracking-widest shadow-lg hover:bg-emerald-600 hover:-translate-y-1 transition-all mb-3">Chat WhatsApp</button>
        ...
        <nav className="pointer-events-auto bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[2rem] px-4 py-3 flex items-center justify-between transition-all duration-500 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
          <div className="flex items-center gap-3.5">
            ...
            <div className="flex flex-col justify-center">
              <span className="text-[17px] font-black tracking-tight text-slate-800 leading-none">SIMantu<span className="text-emerald-500">.</span></span>
          ...
          <div className="flex items-center gap-2">
            <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-full border border-slate-100"><Calendar size={14} className="text-emerald-500" /><span className="text-[12px] font-bold text-slate-600">{data?.periode || "Periode -"}</span></div>
            <div className={`flex items-center gap-2 px-3.5 py-2 rounded-full border transition-all duration-500 ${isOnline ? "bg-emerald-50 border-emerald-100 text-emerald-600" : "bg-rose-50 border-rose-100 text-rose-600"}`}>{isOnline ? <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" /> : <WifiOff size={14} />}<span className="text-[11px] font-bold hidden sm:inline">{isOnline ? "Terhubung" : "Offline"}</span></div>
          </div>
        </nav>
      </div>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-5 sm:px-8 pt-40 sm:pt-48 pb-20 relative z-20">
        ...
        <div className="mb-12 max-w-3xl text-center md:text-left mx-auto md:mx-0 flex flex-col items-center md:items-start animate-fade-slide-up">
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mb-8">
            <div className="flex bg-slate-200/50 p-1.5 rounded-full border border-slate-200/60 shadow-inner backdrop-blur-sm">
              <Link href="/" className="flex items-center gap-2 px-5 sm:px-6 py-2.5 bg-white text-emerald-600 rounded-full text-[11px] sm:text-xs font-black uppercase tracking-widest shadow-sm border border-slate-100"><Wallet size={16} /> Tunjangan Kinerja</Link>
              <Link href="/uang-makan" className="flex items-center gap-2 px-5 sm:px-6 py-2.5 text-slate-500 hover:text-slate-800 rounded-full text-[11px] sm:text-xs font-bold uppercase tracking-widest transition-all"><Utensils size={16} /> Uang Makan</Link>
            </div>
            <button onClick={() => { fetchArsip(); setShowArsipModal(true); }} className="flex items-center gap-2 px-5 py-2.5 bg-white text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-full text-[11px] sm:text-xs font-bold uppercase tracking-widest shadow-sm border border-slate-200 transition-all">
              <History size={16} /> Riwayat
            </button>
          </div>

          <h1 className="text-4xl sm:text-5xl md:text-[4.5rem] font-black text-slate-900 tracking-tight leading-[1.1] mb-6 text-balance">Pencairan Tukin, <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-500">Kini Lebih Transparan.</span></h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-16">
          <div className="lg:col-span-2 bg-white rounded-[2.5rem] p-6 sm:p-12 relative overflow-hidden shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] border border-slate-100">
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
              <div className="flex flex-wrap items-center gap-3 mb-8">
                <div className="inline-flex items-center gap-2.5 px-4 py-2 bg-emerald-50 border border-emerald-100 rounded-full shadow-sm relative overflow-hidden"><div className="absolute inset-0 bg-gradient-to-r from-emerald-100/0 via-white/60 to-emerald-100/0 translate-x-[-100%] animate-shimmer" /><span className="relative flex h-2.5 w-2.5"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" /><span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500" /></span><span className="text-[11px] sm:text-xs font-black uppercase tracking-widest text-emerald-700 relative z-10">Posisi Saat Ini</span></div>
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-8 text-center sm:text-left mb-6 sm:mb-0">
                <div key={safeStep} className="w-24 h-24 sm:w-28 sm:h-28 bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-100/60 rounded-[2rem] flex items-center justify-center text-5xl sm:text-6xl shrink-0 shadow-inner animate-float">{currentStepData.icon}</div>
              ...
              <div className="mt-8 sm:mt-14 bg-slate-50 p-6 rounded-3xl border border-slate-100">
                <div className="flex justify-between items-end mb-3"><span className="text-[11px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider">Penyelesaian</span><span className="text-2xl font-black text-emerald-500 tabular-nums">{progressPct}%</span></div>
                <div className="h-4 w-full bg-slate-200/60 rounded-full overflow-hidden p-0.5"><div className="h-full bg-gradient-to-r from-emerald-400 to-teal-500 rounded-full transition-all duration-[1500ms] ease-out relative" style={{ width: `${progressPct}%` }}><div className="absolute inset-0 bg-white/20 w-full animate-pulse"></div></div></div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            {data?.catatan && data.catatan !== "-" && data.catatan.trim() !== "" ? (
              <div className="bg-amber-50 border border-amber-100 rounded-[2rem] p-8 flex-1 flex flex-col justify-center relative overflow-hidden"><div className="bg-white/60 backdrop-blur-sm w-12 h-12 rounded-2xl flex items-center justify-center text-amber-500 mb-4 shadow-sm relative z-10"><BellRing size={20} className="animate-wiggle" /></div><p className="text-xs font-bold uppercase tracking-wider text-amber-700/60 mb-2 relative z-10">Papan Pengumuman</p><p className="font-bold text-amber-900 text-[15px] leading-relaxed relative z-10 text-pretty">{data.catatan}</p></div>
            ) : (
              <div className="bg-white border border-slate-100 rounded-[2rem] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex-1 flex flex-col justify-center"><div className="bg-emerald-50 w-12 h-12 rounded-2xl flex items-center justify-center text-emerald-500 mb-4"><CheckCheck size={20} /></div><p className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Status Sistem</p><p className="font-bold text-slate-800 text-lg">Semua Normal</p></div>
            )}
        ...
        {isDisbursed && (
          <div className="mb-16 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100 rounded-[2rem] p-8 flex flex-col md:flex-row items-center justify-between gap-8 shadow-sm relative overflow-hidden animate-fade-slide-up">
            <div className="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-6">
              <div className="bg-white w-16 h-16 rounded-full flex items-center justify-center shrink-0 shadow-sm"><Sparkles size={28} className="text-emerald-500 animate-pulse" /></div>
              <div>
                <h3 className="text-2xl font-black text-emerald-900 tracking-tight mb-2">Tukin Selesai Diproses! 🎉</h3>
                <p className="font-medium text-emerald-700 text-sm leading-relaxed">SP2D telah terbit. Mutasi rekening akan masuk secara berkala mulai hari ini.</p>
              </div>
            </div>
            ...
            <div className="bg-white/60 backdrop-blur-sm p-5 rounded-3xl border border-white shrink-0 w-full md:w-auto text-center">
              {!hasRated ? (
                <>
                  <p className="text-xs font-bold text-emerald-800 uppercase tracking-widest mb-3">Nilai Transparansi Kami</p>
                  <div className="flex items-center justify-center gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button 
                        key={star} onClick={() => submitRating(star)} 
                        className={`p-2 rounded-full transition-all duration-300 ${rating >= star ? 'text-amber-400 bg-amber-50 scale-110' : 'text-slate-300 hover:text-amber-300 hover:bg-slate-50'}`}
                      >
                ...
                <div className="py-2 animate-fade-slide-up">
                  <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-2"><CheckCheck size={20} /></div>
                  <p className="text-sm font-black text-emerald-900">Terima Kasih!</p>
                  <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mt-1">Penilaian Anda Tersimpan</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="mb-8 flex items-center justify-between px-2"><h3 className="text-base font-black text-slate-800 tracking-tight flex items-center gap-2"><LayoutDashboard size={18} className="text-emerald-500" /> Rincian Alur Proses</h3></div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 relative">
          {STEPS.map((step, index) => {
            ...
            const statusClass = isDone && !isRejected ? "bg-white border-slate-200 shadow-[0_4px_15px_rgb(0,0,0,0.02)]" : (isCurrent || isFixing) && !isRejected ? "bg-white border-emerald-400 ring-4 ring-emerald-50 shadow-[0_15px_40px_-10px_rgba(16,185,129,0.2)] scale-[1.02] z-10" : isRejected ? "bg-rose-50 border-rose-300 ring-4 ring-rose-50" : "bg-slate-50/50 border-slate-200 border-dashed opacity-70"; 

            return (
              <div key={index} className={`relative p-6 rounded-[2rem] border-2 transition-all duration-500 ease-out group ${statusClass}`}>
                {index < STEPS.length - 1 && (index + 1) % 4 !== 0 && (<div className="hidden lg:block absolute top-11 -right-5 w-5 h-[2px] z-0"><div className={`w-full h-full ${isDone ? 'bg-emerald-400' : 'bg-slate-200'}`}></div></div>)}
                <div className="flex items-start justify-between mb-5 relative z-10">
                  <div className={`w-12 h-12 rounded-[1rem] flex items-center justify-center font-black text-base shadow-sm transition-colors duration-500 ${isDone && !isRejected ? "bg-emerald-100 text-emerald-600" : ""} ${(isCurrent || isFixing) && !isRejected ? "bg-emerald-500 text-white shadow-emerald-500/30" : ""} ${isRejected ? "bg-rose-500 text-white" : ""} ${isPending && !isRejected ? "bg-white text-slate-400 border border-slate-200" : ""}`}>{isDone && !isRejected ? <Check size={20} strokeWidth={3} /> : isRejected ? <X size={20} strokeWidth={3} /> : stepNum}</div>
                ...
                <div className="pt-4 border-t border-slate-100/80 flex items-center justify-between">
                  {isCurrent && !data?.is_rejected ? (<div className="flex items-center gap-1.5 text-[10px] font-black text-emerald-600 uppercase tracking-widest bg-emerald-50 px-2.5 py-1.5 rounded-lg w-full justify-center border border-emerald-100"><span className="relative flex h-1.5 w-1.5"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span><span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-500"></span></span>Sedang Diproses</div>) : isDone && !data?.is_rejected ? (<div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest w-full justify-center"><CheckCheck size={14} className="text-emerald-400" /> Selesai</div>) : isFixing ? (<div className="flex items-center gap-1.5 text-[10px] font-black text-rose-600 uppercase tracking-widest bg-rose-50 px-2.5 py-1.5 rounded-lg w-full justify-center border border-rose-100"><X size={12} strokeWidth={3} /> Perbaikan Berkas</div>) : (<div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest w-full justify-center"><Clock size={12} className="opacity-70" /> {step.eta}</div>)}
                </div>
              </div>
            );
          })}
      ...
      <button onClick={() => setShowHelpModal(true)} className="fixed bottom-8 right-8 px-5 py-4 bg-slate-900 text-white rounded-full flex items-center gap-3 shadow-[0_10px_30px_rgb(0,0,0,0.15)] hover:bg-emerald-500 hover:-translate-y-1 hover:shadow-emerald-500/30 transition-all duration-300 ease-out active:scale-95 z-50 group">
        <HelpCircle size={20} />
